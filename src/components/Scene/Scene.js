...
const Scene = ({ settings, selectedPlanet, planetSelected, planetdata }) => {
  ...
  let [desiredCameraPosition, setDesiredCameraPosition] = useState(
    new THREE.Vector3()
  );
  ...
  const [cameraStartPosition, setCameraStartPosition] = useState(
    new THREE.Vector3()
  );
  const [cameraGroundPosition, setCameraGroundPosition] = useState(
    new THREE.Vector3()
  );
  const [cameraGroundPositionUp, setCameraGroundPositionUp] = useState(
    new THREE.Vector3()
  );
  const [earthRadius, setEarthRadius] = useState(0.10001);
  ...
  const swapPerspective = (event) => {
    ...
    if (swappingFrom === "orbit") {
      ...
      moveCameraTo(
        ...
        cameraGroundPosition.z
      );
      moveTargetTo(
        cameraGroundPosition.x + 0.0001,
        cameraGroundPosition.y,
        cameraGroundPosition.z
      );
      ...
      console.log(
        "camera postiion after orbitControls reset: ",
        camera.position
      );
    } else {
  ...
  const handlePlanetSelection = () => {
    ...
    console.log(
      "Moving target and camera at selectedPlanet",
      selectedPlanet.position
    );
    moveTargetTo(x, y, z);
    moveCameraTo(x, y, z - 0.5);
  ...
  const getUserLocation = () => {
    ...
    if (geolocation) {
      geolocation.getCurrentPosition(
        ...
        (error) => {
          alert(
            "Something went wrong. You must allow location data to use some features of this app."
          );
        }
      );
    }
  ...
  useFrame((state, delta) => {
    ...
    if (
      ...
      if (!controlsActive && autoPositionActive) {
        if (
          Math.abs(state.camera.position.x - desiredCameraPosition.x) > 0.001 ||
          Math.abs(state.camera.position.y - desiredCameraPosition.y) > 0.001 ||
          Math.abs(state.camera.position.z - desiredCameraPosition.z) > 0.001
        ) {
          console.log("setting position to destination", desiredCameraPosition);
          state.camera.position.lerp(desiredCameraPosition, 0.02);
        } else {
          // snap camera to exact position
          console.log(
            "snapping position to destination",
    ...
    if (
      ...
      if (
        Math.abs(controlsRef.current.target.x - cameraTarget.x) > 0.01 ||
        Math.abs(controlsRef.current.target.y - cameraTarget.y) > 0.01 ||
        Math.abs(controlsRef.current.target.z - cameraTarget.z) > 0.01
      ) {
        console.log("setting target to new", cameraTarget);
        controlsRef.current.target.lerp(cameraTarget, 0.02);
      } else {
        // snaps controls target to exact target
        console.log(
          "snapping controls to target",
          cameraTarget,
